...
osx linux-pulse linux-jack linux-alsa cygwin osx-rl: egg

win32:
	make -f makefile.win32egg
...
############################ CHUCK HOST TARGETS ################################
CXXSRCS_HOST+= egg.cpp RtAudio/RtAudio.cpp
...
egg: chuck-core $(COBJS_HOST) $(CXXOBJS_HOST)
	$(LD) -o egg $(OBJS) $(LDFLAGS) $(ARCHOPTS)
...
clean: 
	@rm -rf $(wildcard egg egg.exe) *.o *.d $(OBJS) \
